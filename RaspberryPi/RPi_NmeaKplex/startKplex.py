...
logNamePrefix = "~/kplex_"
logNameSuffix = ""
logNameExtension = ".nmea4"
...
def forgeTimestamp() :
    # format yyyymmdd Bash $(date +%Y%m%d)
    import time
    return time.strftime("%Y%m%d")


logName = logNamePrefix + forgeTimestamp() + logNameSuffix + logNameExtension
...
lblTitle = Label(w, text="Demarrer kplex...", fg="navy")
btnRunKplex = Button(w, text="run kplex", command="runKplex")
btnRunKplexLog = Button(w, text='run kplex with LOG', command="runkplexLog")
txtLogName = StringVar()
txtLogName.set(logName)
# lblLogName = Label(w, textvariable=txtLogName, fg='red', bg='white')
entLogName = Entry(w, textvariable=txtLogName, fg='red', bg='white')
btnQuit = Button(w, text="Quitter cet outil, kplex continue en background...", command=w.destroy)

lblTitle.grid(row=0, column=0, padx=5, pady=5, columnspan=2)
btnRunKplex.grid(row=2, column=0, padx=5, pady=5)
btnRunKplexLog.grid(row=4, column=0, padx=5, pady=5)
entLogName.grid(row=4, column=1, padx=5, pady=5)
btnQuit.grid(row=99, column=0, padx=20, pady=5, columnspan=2)
...
def SetDateTime():
    ...
    Texte.set(ts)
    #datetimeSetCmd = 'sudo timedatectl set-ntp false; sudo timedatectl set-ntp false; sudo timedatectl set-time "' + ts + '"; sleep 1; sudo timedatectl set-ntp true;'
    #datetimeSetCmd = 'sudo timedatectl set-ntp false; sudo timedatectl set-ntp false; sudo timedatectl set-time "' + ts + '";'
    print('[' + ts + ']')
    ret = os.popen("sudo timedatectl set-ntp false").read()
    print(ret)
    ret = os.popen('sudo timedatectl set-time "' + ts + '"').read()
    print(ret)
...
LabelTitle = Label(FrameTitle, text="Demarrer kplex...", fg="navy")
