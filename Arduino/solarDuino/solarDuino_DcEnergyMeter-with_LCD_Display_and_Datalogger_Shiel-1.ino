...
/* 6 - SD memory card shield */
    #include<Wire.h>                          /*Load the library for talking over I2C */
...
/* 6 - SD memory card shield */
      ...
      if (currentMillisSD - startMillisSD >= periodSD)
        {
          mySensorData=SD.open("DCEM.txt",FILE_WRITE);                                                            //Open (DC Energy Meter) DCEM.txt on the SD card as a file to write to
          if(mySensorData)                                                                                        //only to do these if data file opened sucessfully
            {
              ...
              mySensorData.print(finalVoltage);
              mySensorData.print(",");                  
              mySensorData.print(finalCurrent2);
              mySensorData.print(",");    
              mySensorData.print(PowerValue);
              mySensorData.print(",");    
              mySensorData.println(FinalEnergyValue);
